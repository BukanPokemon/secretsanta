...
export function Pairing() {
  ...
  const hasEventInfo = !!eventInfo && (
    eventInfo.eventName || eventInfo.eventDate || eventInfo.exchangeDeadline ||
    eventInfo.budgetMin != null || eventInfo.budgetMax != null
  );
              ...
              <AnimatePresence mode="wait">
                {!revealed ? (
                  ...
                  >
                    ...
                    <p className="mt-4 text-sm text-gray-500">{t('pairing.tapToOpen')}</p>
                  </motion.div>
                ) : (
                  <motion.div
                    key="reveal"
                  ...
                  >
                    ...
                    {hasEventInfo && eventInfo && (
                      <div className="mt-6 p-4 bg-gray-50 rounded text-sm text-gray-700 text-center">
                        <h3 className="font-semibold mb-2">{eventInfo.eventName || t('pairing.eventInfoTitle')}</h3>
                        <div className="space-y-1">
                          {eventInfo.eventDate && (
                            <p>{t('settings.eventDate')}: {formatEventDate(eventInfo.eventDate, i18n.language)}</p>
                          )}
                          {eventInfo.exchangeDeadline && (
                            <p>{t('settings.exchangeDeadline')}: {formatEventDate(eventInfo.exchangeDeadline, i18n.language)}</p>
                          )}
                          {(eventInfo.budgetMin != null || eventInfo.budgetMax != null) && (
                            <p>
                              {t('settings.budgetRange')}:{' '}
                              {eventInfo.budgetMin != null && formatBudget(eventInfo.budgetMin, i18n.language)}
                              {eventInfo.budgetMin != null && eventInfo.budgetMax != null && ' – '}
                              {eventInfo.budgetMax != null && formatBudget(eventInfo.budgetMax, i18n.language)}
                            </p>
                          )}
                        </div>
                      </div>
